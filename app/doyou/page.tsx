...
type Deposit        = { id: string; user_id: string; amount: number; daily_earnings: number; screenshot_url: string; status: string; created_at: string; names?: string; profiles: { full_name: string; email: string } };
type Withdrawal     = { id: string; user_id: string; amount: number; fee: number; amount_after_fee: number; phone: string; status: string; created_at: string; names?: string; profiles: { full_name: string; email: string } };
...
export default function AdminPage() {
  ...
  // Package form
  const [pkgForm, setPkgForm]       = useState({ name: "", amount: "", daily: "", duration: "30" });
  ...
  // ─── Auth — uses server-side API route, password never in client bundle ────
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAuthError(data.error || "Wrong password. Try again.");
      }
    } catch {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (wit.data)   setWithdrawals(wit.data as Withdrawal[]);
    if (usr.data)   setUsers(usr.data as User[]);
    if (codes.data) setRedeemCodes(codes.data as RedeemCode[]);
    if (pkgs.data)  setPackages(pkgs.data as Package[]);
    if (pay.data)   setPaymentSettings(pay.data as PaymentSetting[]);
  ...
  async function approveDeposit(dep: Deposit) {
    ...
    const { data: pkg } = await supabase.from("packages").select("duration_days").eq("amount", dep.amount).maybeSingle();
    await supabase.from("investments").insert({
      user_id: dep.user_id, amount: dep.amount, daily_earnings: dep.daily_earnings,
      duration_days: pkg?.duration_days || 30, days_elapsed: 0, active: true,
    });
    ...
    showToast("✅ Deposit approved!"); loadAll();
  }
  ...
  async function savePackage() {
    ...
    if (!name)             { setPkgMsg("Please enter a package name."); return; }
    if (editingPkg) {
      const { error } = await supabase.from("packages").update({ name, amount, daily_earnings: daily, duration_days: duration }).eq("id", editingPkg.id);
      if (error) { setPkgMsg("Error: " + error.message); return; }
      setEditingPkg(null); showToast("✅ Package updated!");
    } else {
      const { error } = await supabase.from("packages").insert({ name, amount, daily_earnings: daily, duration_days: duration, active: true });
      if (error) { setPkgMsg("Error: " + error.message); return; }
      showToast("✅ Package added!");
    }
    setPkgForm({ name: "", amount: "", daily: "", duration: "30" }); setPkgMsg(""); loadAll();
  }

  function startEditPackage(pkg: Package) {
    setEditingPkg(pkg);
    setPkgForm({ name: pkg.name || "", amount: String(pkg.amount), daily: String(pkg.daily_earnings), duration: String(pkg.duration_days || 30) });
  ...
  async function generateAndBroadcastCode() {
    ...
    const { data: codeRow, error: codeErr } = await supabase
      .from("redeem_codes")
      .insert({ code, value, used: false })
      .select()
      .single();

    if (codeErr || !codeRow) {
      showToast("Error creating code: " + codeErr?.message);
    ...
    if (notifErr) {
      showToast("Code created but notifications failed: " + notifErr.message);
    } else {
      showToast(`✅ Code "${code}" sent to all ${users.length} users!`);
    }
    ...
    return (
      <main className="min-h-screen bg-[#050E1F] flex items-center justify-center px-4">
        <div className="bg-[#0A1628] border border-white/10 rounded-2xl p-8 max-w-sm w-full text-center">
          ...
          <p className="text-white/30 text-xs mb-5">Password is verified server-side</p>
          <input
            type="password"
            value={pw}
  ...
  const totalAmountToPay   = pendingWithdrawals.reduce((s, w) => s + (w.amount_after_fee || 0), 0);
  const totalRequested     = pendingWithdrawals.reduce((s, w) => s + (w.amount || 0), 0);
  const totalFees          = pendingWithdrawals.reduce((s, w) => s + (w.fee || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-[#050E1F] text-white flex flex-col lg:flex-row relative">
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        <div className="px-4 lg:px-8 py-5 border-b border-white/10 bg-[#050E1F] sticky top-0 z-30">
        ...
        <div className="px-4 lg:px-8 pt-5">
          <div className="bg-gradient-to-r from-[#0d1f10] via-[#0a1f1a] to-[#0d1a2f] border border-green-500/20 rounded-2xl p-4 mb-6">
            <p className="text-green-400 text-xs font-bold uppercase tracking-widest mb-3">📊 Platform Overview · {users.length} Users</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-orange-500/10 border border-orange-500/25 rounded-xl p-3 text-center col-span-2 sm:col-span-1">
                <p className="text-orange-300 text-xs font-semibold mb-1">💸 Must Pay Now</p>
                <p className="text-orange-400 font-extrabold text-2xl">GHS {totalAmountToPay.toFixed(2)}</p>
                <p className="text-orange-300/50 text-xs mt-0.5">{pendingWithdrawals.length} pending</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-3 text-center">
                <p className="text-white/40 text-xs mb-1">✅ Fees Earned</p>
                <p className="text-white font-bold text-xl">GHS {totalFees.toFixed(2)}</p>
                <p className="text-white/30 text-xs mt-0.5">10% on pending</p>
              </div>
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 text-center">
                <p className="text-blue-300 text-xs mb-1">🏦 In Wallets</p>
                <p className="text-blue-300 font-bold text-xl">GHS {totalUserBalances.toFixed(2)}</p>
                <p className="text-blue-300/40 text-xs mt-0.5">all accounts</p>
              </div>
              <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-center">
                <p className="text-green-400 text-xs mb-1">📥 Total Requested</p>
                <p className="text-green-400 font-bold text-xl">GHS {totalRequested.toFixed(2)}</p>
                <p className="text-green-400/40 text-xs mt-0.5">pending withdrawals</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 lg:px-8 pb-6 max-w-4xl space-y-6">
          ...
          {tab === "deposits" && (
            <div className="space-y-6">
                    ...
                    <div key={d.id} className="bg-[#0A1628] border border-yellow-500/20 rounded-xl p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          {d.names && (
                            <div className="mt-1 inline-flex items-center gap-1 bg-blue-500/10 border border-blue-500/20 rounded-lg px-2 py-0.5">
                              <span className="text-xs">📱</span>
                              <p className="text-blue-300 text-xs font-semibold">MoMo: {d.names}</p>
                            </div>
                          )}
              ...
              <section>
                <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-3">All Deposits ({deposits.length})</h2>
                    ...
                    <div key={d.id} className="bg-[#0A1628] border border-white/10 rounded-xl p-3 flex justify-between items-center text-sm">
                      <div>
                        ...
                        <p className="text-white/40 text-xs">
                          GHS {d.amount}
                          {d.names && <span className="ml-2 text-blue-300/60">· {d.names}</span>}
          ...
          {tab === "withdrawals" && (
            <div className="space-y-6">
              {pendingWithdrawals.length > 0 && (
                <div className="bg-gradient-to-r from-orange-900/30 to-yellow-900/20 border border-orange-500/30 rounded-2xl p-5">
                  <p className="text-orange-400 text-xs font-bold uppercase tracking-widest mb-3">
                    💸 Pending Payout — {pendingWithdrawals.length} request{pendingWithdrawals.length !== 1 ? "s" : ""}
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-black/20 rounded-xl p-3 text-center"><p className="text-white/40 text-xs mb-1">Total Requested</p><p className="text-white font-bold text-lg">GHS {totalRequested.toFixed(2)}</p></div>
                    <div className="bg-black/20 rounded-xl p-3 text-center"><p className="text-white/40 text-xs mb-1">Fees (10%)</p><p className="text-green-400 font-bold text-lg">GHS {totalFees.toFixed(2)}</p></div>
                    <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-3 text-center"><p className="text-orange-300 text-xs font-semibold mb-1">YOU MUST PAY</p><p className="text-orange-400 font-extrabold text-xl">GHS {totalAmountToPay.toFixed(2)}</p></div>
                  </div>
                </div>
              )}
              <section>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {pendingWithdrawals.map(w => (
                    <div key={w.id} className="bg-[#0A1628] border border-orange-500/20 rounded-xl p-4">
                      ...
                      <p className="text-white/40 text-xs mb-1">📞 MoMo: <span className="font-mono text-white/60">{w.phone}</span></p>
                      {w.names && (
                        <div className="mb-2 inline-flex items-center gap-1 bg-blue-500/10 border border-blue-500/20 rounded-lg px-2 py-0.5">
                          <span className="text-xs">👤</span>
                          <p className="text-blue-300 text-xs font-semibold">Account: {w.names}</p>
                        </div>
                      )}
              ...
              <section>
                <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-3">All Withdrawals ({withdrawals.length})</h2>
                <div className="space-y-2">
                  {withdrawals.map(w => (
                    <div key={w.id} className="bg-[#0A1628] border border-white/10 rounded-xl p-3 flex justify-between items-center text-sm">
                      <div>
                        ...
                        <p className="text-white/40 text-xs">
                          GHS {w.amount} → GHS {w.amount_after_fee?.toFixed(2)}
                          {w.names && <span className="ml-2 text-blue-300/60">· {w.names}</span>}
              ...
              <div className="bg-[#0A1628] border border-white/10 rounded-2xl p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                  <div className="sm:col-span-2">
                    <label className="text-white/50 text-xs block mb-1">Package Name <span className="text-red-400">*</span></label>
                    <input type="text" value={pkgForm.name} onChange={e => setPkgForm({ ...pkgForm, name: e.target.value })} placeholder="e.g. Gold Plan, Starter Package" className={INPUT} />
                  </div>
                  ...
                  <div className="sm:col-span-2"><label className="text-white/50 text-xs block mb-1">Duration (Days)</label><input type="number" value={pkgForm.duration} onChange={e => setPkgForm({ ...pkgForm, duration: e.target.value })} placeholder="e.g. 30" className={INPUT} /></div>
                </div>
                <div className="flex gap-2">
                  ...
                  {editingPkg && <button onClick={() => { setEditingPkg(null); setPkgForm({ name: "", amount: "", daily: "", duration: "30" }); setPkgMsg(""); }} className="px-6 py-3 border border-white/20 rounded-xl text-sm transition">Cancel</button>}
                </div>
              </div>
              <div>
                  ...
                  {packages.map(pkg => (
                    <div key={pkg.id} className={`bg-[#0A1628] border rounded-xl p-4 ${pkg.active ? "border-white/10" : "border-white/5 opacity-50"}`}>
                      <div className="flex justify-between items-start mb-1">
                        <div><p className="font-bold">{pkg.name || "—"}</p><p className="text-white/50 text-sm">GHS {pkg.amount}</p></div>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${pkg.active ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40"}`}>{pkg.active ? "Active" : "Hidden"}</span>
                      </div>
                      ...
                      <div className="flex gap-1.5 mt-3">
                        <button onClick={() => startEditPackage(pkg)} className="flex-1 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-xs transition">✏️ Edit</button>
            ...
            <div className="space-y-6">
              <div className="bg-[#0A1628] border border-white/10 rounded-2xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-white/50 text-xs block mb-1">Network</label>
          ...
          {tab === "users" && (
            <div>
              <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">{users.length} Total Users · GHS {totalUserBalances.toFixed(2)} in wallets</h2>
              <div className="space-y-2">
                {users.map(u => (
            ...
            <div className="space-y-6">
              {todayCode ? (
                <div className="bg-gradient-to-r from-purple-900/30 to-indigo-900/20 border border-purple-500/30 rounded-2xl p-6 text-center">
                  ...
                  <p className="text-white/30 text-xs mt-1">Sent to {users.length} users</p>
                  <button onClick={() => deleteRedeemCode(todayCode.id)} className="mt-4 px-5 py-2 bg-red-500/10 hover:bg-red-500/30 text-red-400 rounded-xl text-xs transition">🗑️ Delete & Invalidate</button>
                </div>
              ) : (
              ...
              <div className="bg-[#0A1628] border border-white/10 rounded-2xl p-5">
                ...
                <p className="text-white/40 text-xs mb-4">All <strong className="text-white/60">{users.length} users</strong> will instantly receive a notification with the code. Each user can only redeem it once within 24 hours.</p>
                {todayCode && (
                  <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 text-yellow-400 text-xs mb-4">
                    ⚠️ A code is already active. Delete it first to generate a new one.
                  </div>
                )}
                <div className="flex gap-2">
                  <div className="flex-1">
                    <label className="text-white/50 text-xs block mb-1">Amount each user receives (GHS)</label>
                    <input type="number" value={newCodeValue} onChange={e => setNewCodeValue(e.target.value)} placeholder="e.g. 20"
                      disabled={!!todayCode} className={INPUT + (todayCode ? " opacity-40 cursor-not-allowed" : "")} />
                  </div>
